...
test("a typed micro nudge stays small for ordinary nudge wording", () => {
  ...
  const result = applyPatternAnswers(state, answer, "nudge that snare earlier");
  assert.equal(result.state.pattern.notes[0].tick, 941);
  const later = applyPatternAnswers(state, noReset(modify("snare_1", 0.9, "later_10ms")), "nudge it a little later");
  assert.equal(later.state.pattern.notes[0].tick, 979);
});
