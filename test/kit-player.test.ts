...
const pattern = (kit: string): Pattern => ({ kit_id: kit, bars: 1, slots_per_bar: 16, notes: [1, 16].map((slot, index) => ({ id: `note_${index + 1}`, instrument: "kick", slot, bar: 1, velocity_layer: 4 })) });
...
test("closed hats choke an old-kit open hat across a kit boundary", async t => {
  const h = harness(t);
  const player = createPatternPlayer();
  const active: Pattern = { ...pattern("tr_808"), notes: [{ id: "note_1", instrument: "open_hat", slot: 16, bar: 1, velocity_layer: 4 }] };
  const next: Pattern = { ...pattern("tr_505"), notes: [{ id: "note_2", instrument: "closed_hat", slot: 1, bar: 1, velocity_layer: 4 }] };
