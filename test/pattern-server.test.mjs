...
import { requestBudgetError } from "../src/ai/request-budget.mjs";
...
test("a large groove narrows to an exact late-bar note before building edit questions", async () => {
  ...
  await withServer(async url => {
    ...
    assert.deepEqual(result.traversal.map(item => item.node), ["edit_scope", "edit_target"]);
  }, { fetchImpl: async (_url, options) => {
    const body = JSON.parse(options.body);
    assert.equal(requestBudgetError(body.state, body.questions), null);
    calls.push(Object.keys(body.questions));
    const answers = answerQuestions(body.questions);
...
test("pattern proxy accepts two hidden articulations at one lane position", async () => {
  ...
  await withServer(async url => {
    ...
    assert.equal(response.status, 200);
  }, { fetchImpl: async (_url, options) => Response.json({ model: "test-jev", answers: answerQuestions(JSON.parse(options.body).questions), usage: {} }) });
});
