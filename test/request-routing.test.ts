...
test("root categories have stable IDs and explicit next nodes", () => {
  assert.deepEqual(Object.keys(REQUEST_CATEGORIES).sort(), ["recorded_rhythm", "fill_rhythm", "edit_pattern", "change_kit", "change_swing", "change_tempo", "undo", "load_preset", "clear_pattern", "clear_selection", "duplicate", "shuffle_preset", "unsupported"].sort());
  assert.equal(resolveRoot("clear_selection").next_node, "selection_clear");
