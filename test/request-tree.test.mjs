...
const decision = value => ({ answers: { selection: { type: "choice", choice: value, probabilities: { [value]: 1 }, confidence: 1 } }, model: "test", latency_ms: 10, question_count: 1, usage: { input_tokens: 5 } });
...
test("kit branch preserves notes, skips note editor and records routing usage", async () => {
  ...
  const completed = await runPatternCommand({ initialState, request: "use 808", decideNode: async (node, state) => {
    visited.push(node);
    assert.equal(state.pattern, undefined);
