...
const notes = [{ id: "note_1", instrument: "kick", bar: 1, slot: 1, velocity_layer: 4 }];
...
test("kit selection survives saved-state migration, resize and note edits", () => {
  ...
  const edited = applyPatternAnswers(state, { reset_pattern: { noul: 1 } });
...
test("kit catalog maps every instrument and velocity to bundled samples", async () => {
  ...
  for (const kit of KITS) {
    for (const instrument of ["kick", "snare", "closed_hat", "open_hat"]) {
      let previous = 0;
