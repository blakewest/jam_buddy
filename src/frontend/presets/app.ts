...
const list = document.querySelector<HTMLElement>("#preset-list")!;
const styleFilter = document.querySelector<HTMLSelectElement>("#style-filter")!;
const resultCount = document.querySelector<HTMLElement>("#result-count")!;
let decisions: Record<string, string> = {};

try { decisions = JSON.parse(localStorage.getItem(storageKey) ?? "{}") ?? {}; } catch { decisions = {}; }

function save(id: string, decision: string | undefined) {
...
function render() {
  ...
  list.replaceChildren(...grooves.map(groove => {
    ...
    for (const button of article.querySelectorAll<HTMLButtonElement>(".actions button")) button.addEventListener("click", () => save(groove.id, button.dataset.choice));
    const audio = article.querySelector("audio")!;
    article.querySelector<HTMLButtonElement>(".preview")!.addEventListener("click", event => {
