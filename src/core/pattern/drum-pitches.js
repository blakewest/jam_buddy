...
export const defaultPitch = instrument => DEFAULT_PITCH[instrument];
export const pitchForNote = note => note.midi_pitch ?? defaultPitch(note.instrument);
export const instrumentForPitch = pitch => Object.keys(PITCHES).find(instrument => PITCHES[instrument].includes(pitch)) ?? null;
export const validPitchForInstrument = (pitch, instrument) => Number.isInteger(pitch) && PITCHES[instrument]?.includes(pitch) === true;
...
export const sampleFamily = pitch => SAMPLE_FAMILY[pitch] ?? null;
