...
import type { TimingState } from "../core/timing/session.js";
import type { ServerResponse } from "node:http";
type Question = { type: string; criteria: Record<string, unknown> };
type ValidatedAnswer = { type: string; choice?: string; noul?: number; probabilities?: Record<string, number> };
type JevResponse = { model: string; answers: Record<string, ValidatedAnswer>; usage?: Record<string, number> };
...
const PUBLIC_FILES = new Map<string, [URL, string]>([
  ...
  ["/frontend/pattern/app.js", staticFile("../frontend/pattern/app.js", "text/javascript")],
  ["/frontend/pattern/audio.js", staticFile("../frontend/pattern/audio.js", "text/javascript")],
  ["/frontend/timing/app.js", staticFile("../frontend/timing/app.js", "text/javascript")],
  ["/frontend/timing/audio.js", staticFile("../frontend/timing/audio.js", "text/javascript")],
...
const MODULE_ALIASES: Record<string, string> = {
  "/app.js": "/frontend/timing/app.js",
  "/timing-audio.js": "/frontend/timing/audio.js",
  "/pattern-app.js": "/frontend/pattern/app.js",
  "/pattern-audio.js": "/frontend/pattern/audio.js",
};

const isRecord = (value: unknown): value is Record<string, unknown> => value !== null && typeof value === "object" && !Array.isArray(value);
const exactKeys = (obj: unknown, keys: readonly string[]): obj is Record<string, unknown> => isRecord(obj) && Object.keys(obj).length === keys.length && keys.every(key => Object.hasOwn(obj, key));
const finite = (value: unknown): value is number => typeof value === "number" && Number.isFinite(value);
const integer = (value: unknown): value is number => typeof value === "number" && Number.isInteger(value);
const midiValue = (value: unknown): value is number => integer(value) && value >= 0 && value <= 127;

function validState(state: unknown): state is TimingState {
  ...
  if (!finite(silence) || silence < 0 || silence > 600000 || (state.drums.status !== "playing" && state.drums.status !== "stopped") || typeof state.drums.scheduled_start !== "boolean" || !Array.isArray(events) || events.length > 500) return false;
  let previous = -1;
  return events.every((event: unknown) => {
    if (!isRecord(event) || !finite(event.time_ms) || event.time_ms < 0 || event.time_ms < previous || event.time_ms > 600000) return false;
    previous = event.time_ms;
    if (event.type === "sustain") return exactKeys(event, ["time_ms", "type", "value"]) && midiValue(event.value);
    return exactKeys(event, ["time_ms", "type", "note", "velocity"]) && (event.type === "note_on" || event.type === "note_off") && midiValue(event.note) && midiValue(event.velocity);
  });
}

function validPatternState(state: unknown): state is PatternJevState {
  ...
  if (!exactKeys(state.pattern, ["bars", "slots_per_bar", "parts"]) || !integer(state.pattern.bars) || state.pattern.bars < 1 || state.pattern.bars > 4 || state.pattern.slots_per_bar !== 16 || !exactKeys(state.pattern.parts, INSTRUMENTS)) return false;
  ...
  for (const instrument of INSTRUMENTS) {
    ...
    for (const note of state.pattern.parts[instrument]) {
      if (!exactKeys(note, ["id", "bar", "position", "velocity_layer"]) || typeof note.id !== "string" || !/^note_[1-9]\d*$/.test(note.id) || ids.has(note.id) || !integer(note.bar) || note.bar < 1 || note.bar > state.pattern.bars || !POSITIONS.includes(note.position as typeof POSITIONS[number]) || !integer(note.velocity_layer) || note.velocity_layer < 1 || note.velocity_layer > 5) return false;
  ...
  return state.recent_history.every((entry: unknown) => exactKeys(entry, ["request", "applied_changes", "rejected_changes"])
...
function validPatternAnswers(data: unknown, questions: Record<string, Question>): data is JevResponse {
  if (!isRecord(data) || typeof data.model !== "string") return false;
  const answers = data.answers;
  if (!exactKeys(answers, Object.keys(questions))) return false;
  return Object.entries(questions).every(([key, question]) => {
    const answer = answers[key];
    if (!isRecord(answer) || answer.type !== question.type) return false;
    if (question.type === "noul") return finite(answer.noul) && answer.noul >= 0 && answer.noul <= 1;
    return typeof answer.choice === "string" && Object.hasOwn(question.criteria, answer.choice)
      && isRecord(answer.probabilities)
      && Object.values(answer.probabilities).every(value => finite(value) && value >= 0 && value <= 1);
  });
}
...
export function createServer({ apiKey = process.env.TYPESAFE_API_KEY, fetchImpl = fetch } = {}) {
  return httpServer(async (req, res) => {
    ...
    const publicFile = PUBLIC_FILES.get(path);
    if (req.method === "GET" && publicFile) {
      const [file, contentType] = publicFile;
      try {
    ...
    try {
      ...
      if (patternRequest) {
        ...
        if (planRequest) {
          ...
          const phraseBars = phraseChoice === "keep_current" ? (payload.state as PatternJevState).pattern.bars : requestedBars;
          if (!Number.isInteger(phraseBars)) return json(res, 502, { error: "TypeSafe returned an invalid phrase length." });
          const scores = INSTRUMENTS.map(instrument => [instrument, data.answers[`involves_${instrument}`].noul ?? 0] as const);
